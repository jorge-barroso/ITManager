...
const std::unordered_map<std::string, Regions::Region> RegionsByName = { { "CA_CENTRAL_1", Regions::Region::CA_CENTRAL_1 },
...
std::string Regions::getRegionName(const Regions::Region region)
{
	return region>=RegionsNames.size() ? Regions::RegionsNames[Regions::Region::NO_REGION] : RegionsNames[region];
}

Regions::Region Regions::getRegionByName(const std::string & name)
{
